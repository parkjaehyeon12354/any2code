/* AI 크레딧 — 모델마다 질문 1번에 고정으로 뺀다(할당제).
   값을 잘못 잡으면 대회 시연 중에 크레딧이 떨어지거나, 반대로 한도가 사실상
   없는 것이 된다. 질문 1번 ≈ 3,000 토큰, 3시간마다 2,000 크레딧. */
...
const { MODEL_COST, FREE_CREDITS } = credit;

test('3시간마다 2,000 크레딧 — Solar 만 20회, Gemini 만 6회', () => {
  assert.strictEqual(FREE_CREDITS, 2000);
  assert.strictEqual(Math.floor(FREE_CREDITS / MODEL_COST.solar), 20);
  assert.strictEqual(Math.floor(FREE_CREDITS / MODEL_COST.gemini), 6);
});

test('Solar 가 기본(첫 항목)이고 Gemini 가 더 비싸다', () => {
  assert.strictEqual(Object.keys(MODEL_COST)[0], 'solar');
  assert.ok(MODEL_COST.gemini > MODEL_COST.solar, '토큰 단가가 약 3배인 모델이 더 싸면 안 된다');
});

test('모든 모델 값은 양의 정수다 — 0 이면 무한정 쓸 수 있는 구멍이 된다', () => {
  for (const [m, c] of Object.entries(MODEL_COST)) {
    assert.ok(Number.isInteger(c) && c > 0, `${m}: ${c}`);
  }
});
...
test('한도 도달은 402로 알린다 — 401/403 과 구분되어야 한다', () => {
  ...
  assert.ok(/remaining < cost\)[\s\S]{0,200}status: 402/.test(src),
    '잔액이 모델 값보다 적으면 402 를 반환해야 한다');
...
test('차감은 답변을 받은 뒤에 한다', () => {
  ...
  assert.ok(callAt > 0 && consumeAt > callAt,
    '호출이 실패하면 빼지 않도록 callLLM 뒤여야 한다');
});
