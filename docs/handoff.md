...
**답변 수정·삭제가 커밋 대기 중입니다. 실패 중인 테스트는 없습니다.**
...
**검사 —** API 16개(권한·상태 전이·답변 수·킬 스위치)와 화면 8개를 붙였고,
**고친 부분을 일부러 되돌려 빨개지는 것까지 확인했습니다.** `prevStatus` 를
되돌리면 2개, 화면 쪽 세 가지 버그는 각각 해당 검사 1개가 정확히 실패합니다.

`test/fake-container.js` 의 가짜 `patch` 가 `incr`/`set` 만 처리하고 있어
`replace`/`add`/`remove` 를 더했습니다 — 실제 Cosmos 는 원래 지원하는 연산이라,
없으면 테스트만 통과하고 실서버는 다르게 돕니다.
