...
마지막 갱신: 2026-08-24 · 배포된 커밋 `09dec5d`
...
아무것도 안 나오고 `pass 173`이면 이 문서와 같은 상태입니다.
...
**3. 관리자 크레딧 지급이 화면에 없다**

`credit.grant(sub, amount)` 는 만들어 뒀는데 관리자 페이지에 연결하지 않았습니다.
지금은 3시간마다 자동으로 채워지므로 급하지는 않습니다.
