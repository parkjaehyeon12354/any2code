...
(function () {
  ...
  const WEIGHTS = [
    ...
    ['#232c52', 16],  // 네이비 라이트
    ['#3d4642', 13],  // 쿨 그레이 — 캔버스 팔레트로 이어주는 다리
    ['#2f4bd6',  9],  // 블루
    ['#167a63',  8],  // 틸 — 브랜드 프라이머리, CTA·로고와 같은 색
    ['#d97706',  6],  // 앰버 — 로고 점과 같은 색
    ['#c2410c',  5],  // 러스트
    ['#8a93b8',  3],  // 라이트 블루그레이 — 가장 먼 안개
  ];
  const PALETTE = WEIGHTS.flatMap(([c, n]) => Array(n).fill(c));
  // 채도 있는 색은 밝은 배경에서 묻히므로 알파를 올려줘야 색으로 읽힌다
  const ACCENTS = new Set(['#2f4bd6', '#167a63', '#d97706', '#c2410c']);
